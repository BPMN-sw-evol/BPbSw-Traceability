...
function evaluateUserTask(element, lines) {
  ...
  lines.push(tooltipLineText('Form Fields', element.businessObject.formFields));
  lines.push(tooltipLineText('Variables', ""));

  try {
    for (let i = 0; i < element.businessObject.extensionElements.values[0].fields.length; i++) {
      lines.push(tooltipLineVariable('', element.businessObject.extensionElements.values[0].fields[i].id));
    }
  } catch (error) {
    console.log(error);
  }
