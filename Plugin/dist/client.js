...
function evaluateUserTask(element, lines) {
  ...
  lines.push((0,_GeneralServiceModule__WEBPACK_IMPORTED_MODULE_0__.tooltipLineText)('Form Fields', element.businessObject.formFields));
  lines.push((0,_GeneralServiceModule__WEBPACK_IMPORTED_MODULE_0__.tooltipLineText)('Variables', ""));

  try {
    for (let i = 0; i < element.businessObject.extensionElements.values[0].fields.length; i++) {
      lines.push((0,_GeneralServiceModule__WEBPACK_IMPORTED_MODULE_0__.tooltipLineVariable)('', element.businessObject.extensionElements.values[0].fields[i].id));
    }
  } catch (error) {
    console.log(error);
  }
...
/* harmony export */   "findVariablesByType": () => (/* binding */ findVariablesByType),
...
/* harmony export */   "tooltipLineVariable": () => (/* binding */ tooltipLineVariable)
/* harmony export */ });
...
function findVariablesByType(element) {
  if (!checkExtensionElementsAvailable(element))
    return undefined;

  console.log(element.businessObject.extensionElements.values[0].fields);

  return findExtension(element.businessObject.extensionElements.values[0]);
}
...
/**
 * add a single tooltip line as 'variable'
 */
function tooltipLineVariable(key, value) {
  return tooltipLineVariableCss(key, value, 'tooltip-value-text variable');
}
 ...
 * like: <div><span>key: </span><span class="css">value</span></div>
 */
function tooltipLineVariableCss(key, value, css) {
  if (value === undefined) return '';
  return `<div class="tooltip-line variable"><span class="tooltip-key">${key}:&nbsp;</span><span class="tooltip-value ${css}">${value}</span></div>`;
}
...
function TooltipInfoService(eventBus, overlays, elementRegistry, editorActions, canvas) {
  ...
  function addListener(element, tooltipId) {
    $('[data-element-id="' + element.id + '"]')
      .on('click', function () { $('#' + tooltipId).show();})
      .on('dblclick', function () { $('#' + tooltipId).hide(); });
  }
